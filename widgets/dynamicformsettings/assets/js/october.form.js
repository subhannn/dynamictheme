...
+function ($) { "use strict";
    ...
    FormWidget.prototype.bindParentTrigger = function() {
        this.$el.on('oc.triggerOn.afterUpdate', '[data-trigger]', this.proxy(this.onUpdateTrigger))
    }

    FormWidget.prototype.onUpdateTrigger = function(e, status){
        var $target = $(e.target)
        if($target.hasClass('hide')){
            var id = $target.attr('id').replace('-group', '')
            $('#'+id, $target)
